...
float convertHHMMtoHours(int hhmm) {
    int hours = hhmm / 100;
...
void reachableDestinationsHours(string sourceFAA, int arrivalTime, int maxValue) {
    ...
    vector<float> minFlightTime(airports.size(), FLT_MAX); // Use float for accurate comparison

    float startTime = convertHHMMtoHours(arrivalTime);
    ...
    while (!q.empty()) {
        ...
        float currTime = convertHHMMtoHours(state.currentTime);
        float totalTimeSoFar = state.totalFlightTime;

        if (state.airportIndex != srcIndex && totalTimeSoFar <= maxValue) {
            reachable[state.airportIndex] = true;
        }

        for (int next = 0; next < airports.size(); ++next) {
            for (const Content& flight : flightGraph[state.airportIndex][next]) {
                float flightStart = convertHHMMtoHours(flight.starttime);
                float flightLand = convertHHMMtoHours(flight.landingtime);
                float flightDuration = flightLand - flightStart;

                if (flightStart >= currTime) {
                    float waitTime = flightStart - currTime;
                    float newTotalTime = totalTimeSoFar + waitTime + flightDuration;

                    cout << "From " << airports[state.airportIndex].FAA << " to " << airports[next].FAA
                         << ": wait " << waitTime << "h, flight " << flightDuration << "h, total " << newTotalTime << "h" << endl;

                    if (newTotalTime < maxValue && newTotalTime < minFlightTime[next]) {
                        minFlightTime[next] = newTotalTime;
                        q.push(Timer(next, flight.landingtime, newTotalTime));
                    }
                }
            }
        }
    ...
    cout << "Reachable airports within " << maxValue << " total hours (wait + flight):" << endl;
    for (int i = 0; i < airports.size(); ++i) {
        if (i != srcIndex && minFlightTime[i] <= maxValue) {
            cout << airports[i].FAA << " - " << airports[i].name << " (" << minFlightTime[i] << " hrs)" << endl;
        }
    }
}
...
void earliestArrival(string start, string dest, int startTimeHHMM) {
    int nodes = airports.size();

    if (airportIndex.find(start) == airportIndex.end() || airportIndex.find(dest) == airportIndex.end()) {
    ...
    vector<float> arrivalTime(nodes, FLT_MAX);
    vector<bool> lockedin(nodes, false);
    vector<int> previous(nodes, -1);

    arrivalTime[begin] = convertHHMMtoHours(startTimeHHMM);
    ...
    for (int i = 0; i < nodes; i++) {
        ...
        for (int j = 0; j < nodes; j++) {
            if (!lockedin[j] && arrivalTime[j] < minTime) {
                minTime = arrivalTime[j];
                currentNode = j;
            }
        }
        ...
        for (int neighbor = 0; neighbor < nodes; neighbor++) {
            for (const auto& flight : flightGraph[currentNode][neighbor]) {
                float flightStart = convertHHMMtoHours(flight.starttime);
                float flightEnd = convertHHMMtoHours(flight.landingtime);

                // Only consider flights that depart after we arrive
                if (flightStart >= arrivalTime[currentNode]) {
                    if (flightEnd < arrivalTime[neighbor]) {
                        arrivalTime[neighbor] = flightEnd;
                        previous[neighbor] = currentNode;
                    }
                }
            }
    ...
    if (arrivalTime[end] == FLT_MAX) {
        cout << "No available path from " << start << " to " << dest << " after " << startTimeHHMM << endl;
    } else {
        cout << "Earliest arrival time at " << dest << ": " << arrivalTime[end] << " hours" << endl;
        ...
        while (!path.empty()) {
            int idx = path.top(); path.pop();
            cout << count++ << "). " << airports[idx].FAA << " - " << airports[idx].name << endl;
        }
    }
}
...
void lastdijkstra(string start, string middle, string dest, int arrivalTime, int type) {
    int nodes = airports.size();
    int begin = airportIndex[start];
    int mid = airportIndex[middle];
    int end = airportIndex[dest];

    // Earliest arrival time at each airport (initialized to infinity)
    vector<int> earliestArrival(nodes, INT_MAX); 
    vector<bool> lockedin(nodes, false);
    vector<int> previous(nodes, -1);

    earliestArrival[begin] = arrivalTime; // Start with the arrival time at the source

    // Dijkstra from source to middle (start -> middle)
    for (int i = 0; i < nodes; i++) {
        int minArrivalTime = INT_MAX;
        int currentNode = -1;

        // Find the airport with the earliest arrival time that hasn't been locked in
        for (int j = 0; j < nodes; j++) {
            if (!lockedin[j] && earliestArrival[j] < minArrivalTime) {
                minArrivalTime = earliestArrival[j];
                currentNode = j;
            }
        }

        if (currentNode == -1) break; // No more reachable airports

        lockedin[currentNode] = true;

        // Traverse neighbors (flights from current airport to others)
        for (int neighbor = 0; neighbor < nodes; neighbor++) {
            if (!flightGraph[currentNode][neighbor].empty() && !lockedin[neighbor]) {
                int newArrivalTime = earliestArrival[currentNode];

                // Loop through each flight option to get the optimal metric (depending on type)
                for (const auto& flight : flightGraph[currentNode][neighbor]) {
                    if (flight.starttime >= newArrivalTime) { // Check if the flight departs after arrival
                        int waitTime = (flight.starttime - newArrivalTime); // Calculate wait time
                        int finalArrivalTime = flight.landingtime + waitTime;

                        // Depending on the type, choose the appropriate metric
                        if (type == 1) { // Fewest flights
                            if (earliestArrival[neighbor] > finalArrivalTime) {
                                earliestArrival[neighbor] = finalArrivalTime;
                                previous[neighbor] = currentNode;
                            }
                        } else if (type == 2) { // Earliest arrival
                            if (finalArrivalTime < earliestArrival[neighbor]) {
                                earliestArrival[neighbor] = finalArrivalTime;
                                previous[neighbor] = currentNode;
                            }
                        } else if (type == 3) { // Cheapest trip
                            int cost = flight.cost; // Use cost for cheapest trip
                            if (earliestArrival[neighbor] > finalArrivalTime && cost < earliestArrival[neighbor]) {
                                earliestArrival[neighbor] = finalArrivalTime;
                                previous[neighbor] = currentNode;
                            }
                        }
    ...
    if (earliestArrival[mid] == INT_MAX) {
        cout << "No path from " << start << " to " << middle << endl;
        return;
    }

    // Second part: from middle to destination (middle -> dest)
    vector<int> earliestArrival2(nodes, INT_MAX);
    vector<bool> lockedin2(nodes, false);
    vector<int> previous2(nodes, -1);

    earliestArrival2[mid] = earliestArrival[mid]; // Start with arrival time at middle

    // Dijkstra for the second part (middle -> dest)
    for (int i = 0; i < nodes; i++) {
        int minArrivalTime = INT_MAX;
        int currentNode = -1;
        ...
        // Find the airport with the earliest arrival time that hasn't been locked in
        for (int j = 0; j < nodes; j++) {
            if (!lockedin2[j] && earliestArrival2[j] < minArrivalTime) {
                minArrivalTime = earliestArrival2[j];
                currentNode = j;
            }
        }
        ...
        lockedin2[currentNode] = true;

        // Traverse neighbors (flights from current airport to others)
        for (int neighbor = 0; neighbor < nodes; neighbor++) {
            if (!flightGraph[currentNode][neighbor].empty() && !lockedin2[neighbor]) {
                int newArrivalTime = earliestArrival2[currentNode];
                ...
                for (const auto& flight : flightGraph[currentNode][neighbor]) {
                    if (flight.starttime >= newArrivalTime) { // Check if the flight departs after arrival
                        ...
                        // Depending on the type, choose the appropriate metric
                        if (type == 1) { // Fewest flights
                            if (earliestArrival2[neighbor] > finalArrivalTime) {
                                earliestArrival2[neighbor] = finalArrivalTime;
                                previous2[neighbor] = currentNode;
                            }
                        } else if (type == 2) { // Earliest arrival
                            if (finalArrivalTime < earliestArrival2[neighbor]) {
                                earliestArrival2[neighbor] = finalArrivalTime;
                                previous2[neighbor] = currentNode;
                            }
                        } else if (type == 3) { // Cheapest trip
                            int cost = flight.cost; // Use cost for cheapest trip
                            if (earliestArrival2[neighbor] > finalArrivalTime && cost < earliestArrival2[neighbor]) {
                                earliestArrival2[neighbor] = finalArrivalTime;
                                previous2[neighbor] = currentNode;
                            }
                        }
    ...
    if (earliestArrival2[end] == INT_MAX) {
        cout << "No path from " << middle << " to " << dest << endl;
        return;
    }

    // Final arrival time (sum of both parts)
    int totalArrivalTime = earliestArrival[mid] + earliestArrival2[end];
    cout << "Earliest arrival time from " << start << " to " << middle << " to " << dest << ": " << totalArrivalTime << endl;

    // Reconstruct the path (start -> middle -> dest)
    stack<int> path;
    int at = mid;
    while (at != -1) {
        path.push(at);
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 0) {
		...
		cout << "13) From source to destination, arriving at Time T, earliest arrival\n";
		cout << "14) From source to destination, stopping at M, arriving at Time T, fewest flights\n";
		cout << "15) From source to destination, stopping at M, arriving at Time T, earliest arrival\n";
		cout << "16) From source to destination, stopping at M, arriving at Time T, cheapest trip\n";
        cout << "0) Quit\n";
		...
		else if (choice == 13 )
		{
            ...
            cin >> time;
            earliestArrival(src, dest, time);  // Call the findFlightPath function

		}

		else if (choice == 14 )
		{
			string src, dest, middle;
			int time;
            cout << "Enter the 3-letter FAA code of the source airport: ";
            cin >> src;
            cout << "Enter the 3-letter FAA code of the middle airport: ";
            cin >> middle;
            cout << "Enter the 3-letter FAA code of the destination airport: ";
            cin >> dest;
            cout << "Enter the arrival time: ";
            cin >> time;
//          lastdijkstra(src, middle, dest, time,1);  // Call the findFlightPath function
			dijkstra(src,middle,3);
			dijkstra(middle,dest,3);
		}
		...
		else if (choice == 15 )
		{
            ...
            cout << "Enter the arrival time: ";
            cin >> time;
            lastdijkstra(src, middle, dest, time,2);  // Call the findFlightPath function
		...
		else if (choice == 16 )
		{
            ...
            cout << "Enter the arrival time: ";
            cin >> time;
            lastdijkstra(src, middle, dest, time,3);  // Call the findFlightPath function

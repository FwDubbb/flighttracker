...
void listFlightsFromSource() {
    ...
    for (int i = 0; i < airports.size(); ++i) {
        if (flightGraph[srcIndex][i].time > 0) {
			...
			cout << "Takeoff : " <<flightGraph[srcIndex][i].starttime / 100 << ":00" << endl;
			cout << "Landing : " <<flightGraph[srcIndex][i].landingtime / 100 << ":00"<< endl;
			cout <<"Time taken: "<<flightGraph[srcIndex][i].time / 100<< ":00" << endl << endl;
            count++;
        }
    }

...
int reverse(float hoursFloat) {
    int hours = static_cast<int>(hoursFloat);
    int minutes = static_cast<int>((hoursFloat - hours) * 60 + 0.5); // round to nearest minute
...
int earliestArrival(string start, string dest, int startTimeHHMM) {
    int nodes = airports.size();
	...
	int ans = reverse(arrivalTime[end]);
	return ans;
...
int fewestFlights(string start, string dest, int startTimeHHMM) {
    int nodes = airports.size();
    ...
    vector<int> flightsTaken(nodes, INT_MAX); // minimize number of flights
    vector<float> arrivalTime(nodes, FLT_MAX);
    vector<int> previous(nodes, -1);
    vector<bool> visited(nodes, false);

    flightsTaken[begin] = 0;
    arrivalTime[begin] = convertHHMMtoHours(startTimeHHMM);
    ...
    for (int i = 0; i < nodes; i++) {
        ...
        for (int j = 0; j < nodes; j++) {
            if (!visited[j] && arrivalTime[j] < minTime) {
                minTime = arrivalTime[j];
                currentNode = j;
            }
        }
        ...
        for (int neighbor = 0; neighbor < nodes; neighbor++) {
            for (const auto& flight : flightGraph[currentNode][neighbor]) {
                ...
                if (flightStart >= arrivalTime[currentNode]) {
                    if (flightsTaken[currentNode] + 1 < flightsTaken[neighbor]) {
                        flightsTaken[neighbor] = flightsTaken[currentNode] + 1;
                        arrivalTime[neighbor] = flightEnd;
                        previous[neighbor] = currentNode;
                    }
                }
    ...
    if (flightsTaken[end] == INT_MAX) {
        cout << "No valid path from " << start << " to " << dest << " after " << startTimeHHMM << endl;
        return -1;
    }

    cout << "Fewest number of flights: " << flightsTaken[end] << endl;
...
int minimumCostPath(string start, string dest, int startTimeHHMM) {
    int nodes = airports.size();
    ...
    vector<float> totalCost(nodes, FLT_MAX);   // Track minimum cost
    vector<float> arrivalTime(nodes, FLT_MAX); // Time of arrival at each airport
    vector<int> previous(nodes, -1);
    vector<bool> visited(nodes, false);
    ...
    for (int i = 0; i < nodes; i++) {
        ...
        for (int neighbor = 0; neighbor < nodes; neighbor++) {
            for (const auto& flight : flightGraph[currentNode][neighbor]) {
                ...
                if (flightStart >= arrivalTime[currentNode]) {
                    if (totalCost[currentNode] + cost < totalCost[neighbor]) {
                        totalCost[neighbor] = totalCost[currentNode] + cost;
                        arrivalTime[neighbor] = flightEnd;
                        previous[neighbor] = currentNode;
                    }
                }
    ...
    if (totalCost[end] == FLT_MAX) {
        cout << "No valid path from " << start << " to " << dest << " after " << startTimeHHMM << endl;
        return -1;
    }

    cout << "Minimum total cost: $" << totalCost[end] << endl;
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 0) {
		...
		else if (choice == 13 )
		{
            ...
            cin >> time;
            int lol = earliestArrival(src, dest, time);  // Call the findFlightPath function
		...
		else if (choice == 14 )
		{
            ...
            cout << "Enter the arrival time: ";
            cin >> time;
			int newtime = fewestFlights(src, middle, time);
			int lol = fewestFlights(middle,dest,newtime);
		}
		...
		else if (choice == 15 )
		{
            ...
            cout << "Enter the arrival time: ";
            cin >> time;
			int newtime = earliestArrival(src, middle, time);
			int lol = earliestArrival(middle,dest,newtime);
		}
		...
		else if (choice == 16 )
		{
            ...
            cout << "Enter the arrival time: ";
            cin >> time;
			int newtime = minimumCostPath(src, middle, time);
			int lol = minimumCostPath(middle,dest,newtime);
		}

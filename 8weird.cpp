...
struct BFSState {
    ...
    BFSState(int airportIndex, int currentTime, int flightsTaken)
        : airportIndex(airportIndex), currentTime(currentTime), flightsTaken(flightsTaken) {}
};
...
void reachableDestinations(string sourceFAA, int arrivalTime, int maxFlights) {
    ...
    // Queue holds: current airport, current time, depth (# of flights)
    queue<BFSState> q;

    // Initialize reachable vector with 'false' (default)
    vector<bool> reachable(airports.size(), false);

    // Mark the source airport as reachable (we start from there)
    reachable[srcIndex] = true;

    q.push(BFSState(srcIndex, arrivalTime, 0));

    while (!q.empty()) {
        BFSState state = q.front();
        q.pop();

        if (state.flightsTaken > maxFlights) continue;
        ...
        for (int next = 0; next < airports.size(); ++next) {
            for (const auto& flight : flightGraph[state.airportIndex][next]) {
                if (flight.starttime >= state.currentTime) {
                    // If we haven't visited this airport, mark it as reachable and push it to the queue
                    if (!reachable[next]) {
                        reachable[next] = true;
                        q.push(BFSState(next, flight.landingtime, state.flightsTaken + 1));
                    }
                }
            }
        }
    ...
    // Output the reachable airports
    cout << "Destinations reachable from " << sourceFAA << " after time " << arrivalTime << " within " << maxFlights << " flights:\n";
    bool foundReachable = false;
    for (int i = 0; i < airports.size(); ++i) {
        if (reachable[i] && i!= srcIndex) {
    ...
    if (!foundReachable) {
        cout << "No destinations reachable from " << sourceFAA << " within " << maxFlights << " flights." << endl;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 0) {
		...
		else if (choice == 10 )
		{
            ...
            cin >> flights;
            reachableDestinations(src, time, flights);  // Call the findFlightPath function
